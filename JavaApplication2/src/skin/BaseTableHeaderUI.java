    ...
    protected void paintBackground(Graphics g, Rectangle cellRect, int col) {
        ...
        if (header.getBackground() instanceof ColorUIResource) {
            if ((col == rolloverCol) && (component != null) && component.isEnabled()) {
                Utilities.fillHorGradient(g, BaseLookAndFeel.getTheme().getRolloverColors(), x, y, w, h);
            } else if (drawAlwaysActive() || Utilities.isFrameActive(header)) {
                Utilities.fillHorGradient(g, BaseLookAndFeel.getTheme().getColHeaderColors(), x, y, w, h);
            } else {
                Utilities.fillHorGradient(g, BaseLookAndFeel.getTheme().getInActiveColors(), x, y, w, h);
            }
        } else {
            g.setColor(header.getBackground());
            g.fillRect(x, y, w, h);
...
//----------------------------------------------------------------------------------------------------------------------
    ...
    private class MyRenderComponent extends JLabel {
        ...
        public MyRenderComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int col) {
            ...
            if (col == rolloverCol) {
                setForeground(BaseLookAndFeel.getTheme().getRolloverForegroundColor());
            } else {
            ...
            if ((Utilities.getJavaVersion() >= 1.6) && (UIManager.getLookAndFeel() instanceof BaseLookAndFeel)) {
                ...
                if ((keyList != null) && (keyList.size() > 0)) {
                    ...
                    if (sortKey.getColumn() == table.convertColumnIndexToModel(col)) {
                        BaseIconFactory iconFactory = ((BaseLookAndFeel) UIManager.getLookAndFeel()).getIconFactory();
        ...
        protected void paintBackground(Graphics g) {
            ...
            if (header == null) {
                Utilities.fillHorGradient(g, BaseLookAndFeel.getTheme().getColHeaderColors(), 0, 0, getWidth(), getHeight());
                return;
            }
            int draggedColumn = -1;
            ...
            if ((table != null) && table.isEnabled() && (col == rolloverCol || col == draggedColumn)) {
                Utilities.fillHorGradient(g, BaseLookAndFeel.getTheme().getRolloverColors(), 0, 0, w, h);
                if (drawRolloverBar()) {
                ...
                if (header.getBackground() instanceof ColorUIResource) {
                    Utilities.fillHorGradient(g, BaseLookAndFeel.getTheme().getColHeaderColors(), 0, 0, w, h);
                } else {
                    g.setColor(header.getBackground());
                    g.fillRect(0, 0, w, h);
                }
                ...
                if (header.getBackground() instanceof ColorUIResource) {
                    Utilities.fillHorGradient(g, BaseLookAndFeel.getTheme().getInActiveColors(), 0, 0, w, h);
                } else {
                    g.setColor(header.getBackground());
                    g.fillRect(0, 0, w, h);
                }
