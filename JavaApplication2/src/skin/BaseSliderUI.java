...
public class BaseSliderUI extends BasicSliderUI {
    ...
    public Icon getThumbHorIcon() {
        if (UIManager.getLookAndFeel() instanceof BaseLookAndFeel) {
            return ((BaseLookAndFeel) UIManager.getLookAndFeel()).getIconFactory().getThumbHorIcon();
        }
        return null;
    }
    ...
    public Icon getThumbHorIconRollover() {
        if (UIManager.getLookAndFeel() instanceof BaseLookAndFeel) {
            return ((BaseLookAndFeel) UIManager.getLookAndFeel()).getIconFactory().getThumbHorIconRollover();
        }
        return null;
    }
    ...
    public Icon getThumbVerIcon() {
        if (UIManager.getLookAndFeel() instanceof BaseLookAndFeel) {
            return ((BaseLookAndFeel) UIManager.getLookAndFeel()).getIconFactory().getThumbVerIcon();
        }
        return null;
    }
    ...
    public Icon getThumbVerIconRollover() {
        if (UIManager.getLookAndFeel() instanceof BaseLookAndFeel) {
            return ((BaseLookAndFeel) UIManager.getLookAndFeel()).getIconFactory().getThumbVerIconRollover();
        }
        return null;
    }
    ...
    public void paintBackground(Graphics g, JComponent c) {
        if (c.isOpaque()) {
            if (c.getBackground() instanceof ColorUIResource) {
                g.setColor(BaseLookAndFeel.getBackgroundColor());
            } else {
    ...
    public void paintTrack(Graphics g) {
        boolean leftToRight = Utilities.isLeftToRight(slider);
        ...
        g.setColor(BaseLookAndFeel.getFrameColor());
        ...
        if (slider.getOrientation() == JSlider.HORIZONTAL) {
            ...
            Color colors[];
            if (!Utilities.isActive(slider)) {
                colors = BaseLookAndFeel.getTheme().getInActiveColors();
            } else {
                if (slider.isEnabled()) {
                    colors = BaseLookAndFeel.getTheme().getSliderColors();
                } else {
                    colors = BaseLookAndFeel.getTheme().getDisabledColors();
                }
            }
            Utilities.fillHorGradient(g, colors, fillLeft + 2, fillTop + 2, fillRight - fillLeft - 2, fillBottom - fillTop - 2);
            Color cHi = ColorHelper.darker(colors[colors.length - 1], 5);
            Color cLo = ColorHelper.darker(colors[colors.length - 1], 10);
            Utilities.draw3DBorder(g, cHi, cLo, fillLeft + 1, fillTop + 1, fillRight - fillLeft - 1, fillBottom - fillTop - 1);
        } else {
            ...
            Utilities.fillVerGradient(g, colors, fillLeft + 2, fillTop + 2, fillRight - fillLeft - 2, fillBottom - fillTop - 2);
            Color cHi = ColorHelper.darker(colors[colors.length - 1], 5);
            Color cLo = ColorHelper.darker(colors[colors.length - 1], 10);
            Utilities.draw3DBorder(g, cHi, cLo, fillLeft + 1, fillTop + 1, fillRight - fillLeft - 1, fillBottom - fillTop - 1);
        }
