...
public class BaseMenuUI extends BasicMenuUI {
    ...
    protected void paintBackground(Graphics g, JComponent c, int x, int y, int w, int h) {
        ...
        if (backColor == null || backColor instanceof UIResource) {
            backColor = BaseLookAndFeel.getMenuBackgroundColor();
        }
        ButtonModel model = mi.getModel();
        if (c.getParent() instanceof JMenuBar) {
           if (model.isRollover() || model.isArmed() || (c instanceof JMenu && model.isSelected())) {
                backColor = BaseLookAndFeel.getMenuSelectionBackgroundColor();
                if (model.isRollover()) {
                ...
                g.setColor(BaseLookAndFeel.getMenuSelectionForegroundColor());
            }
        } else {
            if (model.isArmed() || model.isRollover() || (c instanceof JMenu && model.isSelected())) {
                g.setColor(BaseLookAndFeel.getMenuSelectionBackgroundColor());
                g.fillRect(x, y, w, h);
                g.setColor(BaseLookAndFeel.getMenuSelectionForegroundColor());
            } else if (!BaseLookAndFeel.getTheme().isMenuOpaque()) {
                ...
                g2D.setColor(BaseLookAndFeel.getMenuForegroundColor());
            } else {
                g.setColor(backColor);
                g.fillRect(x, y, w, h);
                g.setColor(BaseLookAndFeel.getMenuForegroundColor());
            }
        }
    }
    ...
    protected void paintText(Graphics g, JMenuItem menuItem, Rectangle textRect, String text) {
        ...
        if (menuItem.getParent() instanceof JMenuBar) {
            ...
            if (foreColor instanceof UIResource) {
                foreColor = BaseLookAndFeel.getMenuForegroundColor();
            }
