...
public class BaseToolTipUI extends MetalToolTipUI {
    ...
    public void installUI(JComponent c) {
        super.installUI(c);
        int borderSize = BaseLookAndFeel.getTheme().getTooltipBorderSize();
        int shadowSize = BaseLookAndFeel.getTheme().getTooltipShadowSize();
    ...
    public void paint(Graphics g, JComponent c) {
        ...
        int w = c.getWidth();
        int h = c.getHeight();
        Color backColor = BaseLookAndFeel.getTheme().getTooltipBackgroundColor();
        ...
        if (!mediumWeight && fancyLayout && shadowSize > 0) {
            ...
            int shadowOffset = BaseLookAndFeel.getTheme().isTooltipCastShadow() ? shadowSize : 0;
            ...
            if (ColorHelper.getGrayValue(backColor) < 128) {
                g2D.setColor(ColorHelper.brighter(BaseLookAndFeel.getTheme().getBackgroundColor(), 20));
            } else {
            ...
            if (c instanceof JToolTip) {
                ...
                if (tip.getComponent() != null && tip.getComponent().isEnabled()) {
                    c.setForeground(BaseLookAndFeel.getTheme().getTooltipForegroundColor());
                } else {
                    c.setForeground(BaseLookAndFeel.getTheme().getDisabledForegroundColor());
                }
            }
            super.paint(big, c);
