...
public class BaseScrollBarUI extends BasicScrollBarUI {
    ...
    protected JButton createDecreaseButton(int orientation) {
        if (BaseLookAndFeel.getTheme().isMacStyleScrollBarOn()) {
    ...
    protected void paintTrack(Graphics g, JComponent c, Rectangle trackBounds) {
        ...
        int h = c.getHeight();
        if (BaseLookAndFeel.getTheme().isMacStyleScrollBarOn()) {
            Color bc = ColorHelper.darker(BaseLookAndFeel.getTheme().getBackgroundColor(), 4);
            g.setColor(bc);
            g.fillRect(0, 0, w, h);
        } else {
            if (scrollbar.getOrientation() == JScrollBar.VERTICAL) {
                Utilities.fillVerGradient(g, BaseLookAndFeel.getTheme().getTrackColors(), 0, 0, w, h);
            } else {
                Utilities.fillHorGradient(g, BaseLookAndFeel.getTheme().getTrackColors(), 0, 0, w, h);
            }
        }
    }
    ...
    protected Color[] getThumbColors() {
        if (isRollover || isDragging) {
            return BaseLookAndFeel.getTheme().getRolloverColors();
        } else if (!Utilities.isActive(scrollbar)) {
            return BaseLookAndFeel.getTheme().getInActiveColors();
        } else {
            return BaseLookAndFeel.getTheme().getThumbColors();
        }
    }
    ...
    protected void paintThumb(Graphics g, JComponent c, Rectangle thumbBounds) {
        ...
        if (scrollbar.getOrientation() == JScrollBar.VERTICAL) {
            Utilities.fillVerGradient(g, colors, 1, 1, thumbBounds.width - 1, thumbBounds.height - 1);
            Utilities.draw3DBorder(g, frameColorLo, ColorHelper.darker(frameColorLo, 15), 0, 0, thumbBounds.width, thumbBounds.height);
            ...
            Utilities.fillHorGradient(g, colors, 1, 1, thumbBounds.width - 1, thumbBounds.height - 1);
            Utilities.draw3DBorder(g, frameColorLo, ColorHelper.darker(frameColorLo, 10), 0, 0, thumbBounds.width, thumbBounds.height);
    ...
    protected void layoutVScrollbar(JScrollBar sb) {
        if (BaseLookAndFeel.getTheme().isLinuxStyleScrollBarOn() && incrButton.isVisible() && decrButton.isVisible()) {
